...
class Crypt
{
    ...
    public function decrypt($str){
        $ivlen=\openssl_cipher_iv_length($this->cryptMethod);
        $crypt_Date=$this->cryptUnCombine($str, $ivlen);
        $original_plaintext = openssl_decrypt($crypt_Date['str'], $this->cryptMethod, \CRYPT_KEY, OPENSSL_RAW_DATA, $crypt_Date['iv']);
        $calcmac = hash_hmac($this->hashAlgoritm, $crypt_Date['str'], \CRYPT_KEY, true);
        if(\hash_equals($crypt_Date['hmac'],$calcmac)){
            return $original_plaintext;
        }
        return  false;
    }
    protected function cryptCombine($str, $iv, $hmac): string
    {
        $new_str='';
        $str_len=strlen($str);
    ...
    protected function cryptUnCombine($str, $ivlen): array
    {
        $crypt_data=[];
        ...
        for($i=0; $i<\strlen($str); $i++){
            if($ivlen+\strlen($crypt_data['str'])<\strlen($str)){
                ...
                $crypt_data_str_len=\strlen($crypt_data['str']);
                $crypt_data['str'].=\substr($str, $i, \strlen($str)-$ivlen-$crypt_data_str_len);
                $crypt_data['iv'].=\substr($str, $i+(\strlen($str)-$ivlen-$crypt_data_str_len));
                break;
            }
        }
        return $crypt_data;
